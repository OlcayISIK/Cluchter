...
def app(df, x, y):
    ...
    if "olcay_active_button" not in st.session_state:
        st.session_state.olcay_active_button = "5"
    if "olcay_show_suggestion" not in st.session_state:
        st.session_state.olcay_show_suggestion = False
    ...
    has_suggestion = st.session_state.get("olcay_show_suggestion", False)
    ...
    if has_suggestion and suggestion_col is not None:
        with suggestion_col:
            ...
            show_suggestion(st.session_state.get("olcay_stress_level"))

    if st.session_state.olcay_active_button == "5":
...
def detect_and_display(x, y, features):
    ...
    st.session_state.olcay_stress_level = detection
