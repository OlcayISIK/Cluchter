...
def app(df, x, y):
    ...
    if "kimjiwon_active_button" not in st.session_state:
        st.session_state.kimjiwon_active_button = "5"
    if "kimjiwon_show_suggestion" not in st.session_state:
        st.session_state.kimjiwon_show_suggestion = False

    col1, col2 = st.columns([1, 5])
    with col1:
        st.image("images/kimjiwon.jpeg", use_container_width=True, width=200)
    with col2:
        st.markdown("<div class='big-title'>Kim Ji-won / Controller 🔵</div>", unsafe_allow_html=True)
        st.markdown("<div class='big-title-second'>Current Character: <span class='optimal'>Brimstone</span></div>", unsafe_allow_html=True)              
    ...
    has_suggestion = st.session_state.get("kimjiwon_show_suggestion", False)
    ...
    with slider_col:
        ...
        rem = slider_val("Hand Tremors", "kimjiwon_rem", float(df["rem"].min()), float(df["rem"].max()), float(preset_vals[5]))
    ...
    if has_suggestion and suggestion_col is not None:
        with suggestion_col:
            ...
            show_suggestion(st.session_state.get("kimjiwon_stress_level"))

    if st.session_state.kimjiwon_active_button == "5":
...
def detect_and_display(x, y, features):
    ...
    st.session_state.kimjiwon_stress_level = detection
    st.info("Stress Level Calculated. Reports are being sent.")

def show_suggestion(detection):
    if detection == 1:
        st.success("Low Stress Level")
        st.markdown("<div class='suggestion-box'>- Keep a clear and steady mindset.<br>- Maintain smooth, even breaths.<br>- <b>Stay hydrated throughout the day</b>.</div>", unsafe_allow_html=True)
    elif detection == 2:
        st.warning("Medium Stress Level")
        st.markdown("<div class='suggestion-box'>- Pause briefly to reset your mind.<br>- Stay relaxed and centered.<br>- <b>Rinse your face with cool water</b>.</div>", unsafe_allow_html=True)
    elif detection == 3:
        st.error("High Stress Level!")
        st.markdown("<div class='suggestion-box'>- Try the 4-7-8 breathing method.<br>- <b>Hydrate with cold water</b>.<br>- Loosen up with gentle stretches.</div>", unsafe_allow_html=True)
    elif detection == 4:
        st.error("Very High Stress Level!!")
        st.markdown("<div class='suggestion-box'>- Take a short break to decompress.<br>- Use box breathing for calmness.<br>- <b>Rehydrate with an electrolyte drink</b>.</div>", unsafe_allow_html=True)
    else:
        st.success("Stress Free and Calm")
        st.markdown("<div class='suggestion-box'>- Keep a steady, balanced pace.<br>- <b>Drink water periodically</b>.<br>- Stay alert and rejuvenated.</div>", unsafe_allow_html=True)